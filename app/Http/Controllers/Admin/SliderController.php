...
class SliderController extends Controller
{
    ...
    {
        if ($request->status == '1') {
            $query = $this->slider->where('publish_status', '1');
        } elseif ($request->status == '0') {
            $query = $this->slider->where('publish_status', '0');
        } else {
            $query = $this->slider;
        }
        if ($request->keyword) {
        ...
        if ($request->slider_type) {
            ...
            $query = $query->where('slider_type', $slider_type);
        }
    ...
    public function store(Request $request)
    {
        $this->validate($request, [
            ...
            'np_title' => 'nullable|string|min:3|max:190',
