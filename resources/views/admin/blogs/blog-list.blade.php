                ...
                <div class="card-body search-body">
                    ...
                    <div class="p-1 col-lg-4">
                        <div class="card-tools float-right">
                            @can('blog-create')
                                <a href="{{ route('blog.create') }}" class="global-btn">
                ...
                <table class="table table-bordered"> {{-- table-bordered --}}
                    ...
                    <tbody>
                        @foreach ($data as $key => $value)
                            <tr>
                                <td>{{ $key + 1 }}.</td>
                                <td>{{ $value->title[$_locale] }}</td>
                                <td>{{ ucWords($value->postType) }}</td>
                                <td>
                                    {{-- dd($value) --}}
                                    <img src="{{ getThumbImage($value->featured_img, $value->featured_img_path) }}"
                                        alt="{{ @$value->title[$_locale] }}" class="img img-thumbail" style="width:60px">
                                </td>
                                ...
                                <td>
                                    <div class="btn-group">
                                        <div class="btn-group">
                                            @can('blog-edit')
                                                <a href="{{ route('blog.edit', $value->id) }}" title="Edit Service"
                                                    class="view-btn mr-1"><i class="fas fa-edit"></i></a>
                                            @endcan
                                            @can('blog-delete')
                                                {{ Form::open(['method' => 'DELETE', 'route' => ['blog.destroy', $value->id], 'style' => 'display:inline', 'onsubmit' => 'return confirm("Are you sure you want to delete this Service?")']) }}
                                                {{ Form::button('<i class="fas fa-trash-alt"></i>', ['class' => 'view-btn', 'type' => 'submit', 'title' => 'Delete blog ']) }}
                                                {{ Form::close() }}
                                            @endcan
                                        </div>
                                    </div>
