                    ...
                    <div class="p-1 col-lg-4">
                        <div class="card-tools float-right">
                            @can('information-create')
                                <a href="{{ route('information.create') }}" class="global-btn">
                    ...
                    <tbody>
                        @foreach ($data as $key => $value)
                            <tr>
                                ...
                                <td>{{ $value->title }}</td>
                                <td>
                                    <img src="{{ @$value->information_image }}" alt="{{ @$value->title }}"
                                ...
                                <td>
                                    <div class="btn-group">
                                        @can('information-edit')
                                            <a href="{{ route('information.edit', $value->id) }}" title="Edit Service"
                                                class="view-btn mr-1"><i class="fas fa-edit"></i></a>
                                        @endcan
                                        @can('information-delete')
                                            {{ Form::open(['method' => 'DELETE', 'route' => ['information.destroy', $value->id], 'style' => 'display:inline', 'onsubmit' => 'return confirm("Are you sure you want to delete this Service?")']) }}
                                            {{ Form::button('<i class="fas fa-trash-alt"></i>', ['class' => 'view-btn', 'type' => 'submit', 'title' => 'Delete information ']) }}
                                            {{ Form::close() }}
                                        @endcan
                                    </div>
                                </td>
