                            ...
                            @if($permissions->count()>0)
                                ...
                                <div class="row">
                                    ...
                                    @foreach($permissions as $key=>$value)
                                        <div class="col-md-12">
                                            <strong
                                                class="d-inline-block m-b-5">Manage {{ucfirst(str_replace('_',' ',$key))}}
                                                :</strong>
                                        </div>
                                        @foreach($value as $permission)
                                            <div class="col-md-3">
                                                ...
                                                <div>
                                                    <label>{{Form::checkbox('permission[]', $permission->id, @in_array($permission->id, $rolePermissions) ? true : false, ['class' => 'name']) }} {{ucfirst(str_replace('-',' ',$permission->name))}}</label>
                                                </div>
                                            </div>
                                        @endforeach
                                    @endforeach
